function[output,v_ll_ratio] = BP_changePoint(xData, yData, offset, Length, sigma, confidenceLevel)	
...
null_line = BP_lingress(xData, yData, offset, Length);
null_ll = BP_log_likelihood(xData, yData, offset, Length, null_line(1), null_line(3), sigma);
...
for w = 2:Length - 2
	...
	segA_line = BP_lingress(xData, yData, offset, w);
	%linear fit for second half
	segB_line = BP_lingress(xData, yData, offset + w, Length - w);
			
	ll_ratio = BP_log_likelihood(xData, yData, offset, w, segA_line(1), segA_line(3), sigma) +...
        BP_log_likelihood(xData, yData, offset + w, Length - w, segB_line(1), segB_line(3), sigma) - null_ll;
