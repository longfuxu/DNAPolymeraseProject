function[Segment_cell,CP_cell,varargout] = BP_Batch_Segments_for(xData_cell,yData_cell,varargin)

DefStruct = struct('SavePathName', [],'offset', 1, 'sigma', 1e-10, 'confidencelevel', .999,...
    'SaveFileName', {[]},'k',[],'Inds',0,'Movie',0, 'linear', 1);
...
for i = 1:Ncurves
    ...
    if Movie
        if Args.linear
            ...
            Offset, 'sigma', Sigma,'confidencelevel', Conflevel,'movie',Movie);
        else
            [CP_positions,cphist,llrhist] = BP_binary_search_offset(xData_cell{i}, yData_cell{i}, 'offset',...
            Offset, 'sigma', Sigma,'confidencelevel', Conflevel,'movie',Movie);
        end
        CP_hist(i) = cphist;
        LLR_hist(i) = llrhist;
        ...
        if Args.linear
            ...
            Offset, 'sigma', Sigma,'confidencelevel', Conflevel,'movie',0);
        else
            [CP_positions] = BP_binary_search_offset(xData_cell{i}, yData_cell{i}, 'offset',...
            Offset, 'sigma', Sigma,'confidencelevel', Conflevel,'movie',0);
        end
    end
    ...
    if Args.linear
        ...
        Segment_cell{i} = BP_Segments_offset(xData_cell{i}, yData_cell{i}, CP_positions,...
            'SavePathName',SavePathName,'SaveFileName',SaveFileName{i});
    end
...
if Movie && Nout>1
    varargout(2:3) = [{CP_hist}, {LLR_hist}];
end;
