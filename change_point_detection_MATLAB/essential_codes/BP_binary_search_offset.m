function[CP_positions,varargout] = BP_binary_search_offset(xData, yData, varargin)
...
while i  < length(CP_positions)
    k = i;
    [cp,llr] = BP_changePoint_offset(xData, yData, CP_positions(i), CP_positions(i+1)-CP_positions(i), Args.sigma, Args.confidencelevel);
    if (cp ~= -1)
...
if length(CP_positions) > 2 
    while j < (length(CP_positions) - 1)
        k = j;
        [cp,llr] = BP_changePoint_offset(xData, yData, CP_positions(j), CP_positions(j+2)-CP_positions(j), Args.sigma, Args.confidencelevel);
        CP_positions(j+1) = [];        
